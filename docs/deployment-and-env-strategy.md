...
| **Yerel Postgres (yalnızca DB, Docker)** | Sadece PostgreSQL konteynerı: `infra/dev/docker-compose.postgres.yml`. Kökten `npm run db:dev:up`; `apps/api/.env` içinde örnek `DATABASE_URL`: `postgresql://crm_dev:crm_dev@localhost:5433/crm_dev` (`apps/api/.env.example`). Şema: `cd apps/api && npx prisma migrate deploy`. |
