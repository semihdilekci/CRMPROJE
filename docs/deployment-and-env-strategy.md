...
**Sürüm pinleme (Fortify — Docker Misconfiguration):** API/Web Dockerfile’larında taban imaj **`node:20.20.2-bookworm-slim@sha256:677c9bc72e8f52777aa42241fdb12a0aa87dc23f76299871686443907619f767`** (linux/amd64 manifest’i); `apt` ile kurulan **`openssl`** / **`ca-certificates`** paketleri **tam sürüm** ile sabitlenmiştir. Taban imaj veya Debian repo’su değiştiğinde: `docker buildx imagetools inspect …` ile digest doğrulayın; aynı imajda `apt-cache policy openssl ca-certificates` ile apt pin’lerini güncelleyin. İzleme stack’inde Grafana Postgres: **`postgres:16.13-alpine3.23@sha256:60b180625695e90b6e450405d5f0bcb74bc8c52c65d1cd11aeb3da46cac06dd8`**. Dockerfile sözdizimi: `# syntax=docker/dockerfile:1.12`.
