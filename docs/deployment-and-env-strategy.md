...
| **Web güvenlik başlıkları (Faz 7 sec7-05)** | CSP + middleware; DEV’de HMR için `unsafe-eval` | **HSTS** yalnızca `NODE_ENV=production` (`next build` + `next start` veya canlı); `next dev` ile **HSTS yok** (normal). `Content-Security-Policy`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `X-Frame-Options`, `upgrade-insecure-requests`. **Kartvizit OCR (Tesseract.js):** worker/core/dil dosyaları varsayılan olarak `https://cdn.jsdelivr.net` — CSP’de tek harici kaynak olarak bu origin izinli. Kaynak: `apps/web/src/middleware.ts` + `apps/web/src/lib/security-headers.ts` |
...
- **Web CSP / güvenlik başlıkları (sec7-05):** `apps/web/src/middleware.ts` + `apps/web/src/lib/security-headers.ts` — sayfa isteklerinde CSP, HSTS (yalnızca production), diğer güvenlik başlıkları. Tesseract için `https://cdn.jsdelivr.net` (script-src, connect-src, worker-src). Başlıkları görmek için `next start` veya canlı build; `next dev` HSTS eklemez.
